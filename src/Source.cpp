	...
	for (int circle = 0;circle < circles.size();circle++) {
		...
		for (int other_circle = 0;other_circle < circles.size();other_circle++) {
			...
			if (other_circle != circle) {
				...
				distance[0] = circles[other_circle].getPosition()[0]-position[0];
				distance[1] = circles[other_circle].getPosition()[1] - position[1];

				overlap = sqrt(distance[0] * distance[0] + distance[1] * distance[1]) - (circles[circle].getRadius() + circles[other_circle].getRadius());
				...
				if (overlap<0) {
					...
					//Adjust velocity
					velocity[0] = (velocity[0] + distance[0]) / sqrt(velocity[0] * velocity[0] + distance[0] * distance[0]);
					velocity[1] = (velocity[1] + distance[1]) / sqrt(velocity[1] * velocity[1] + distance[1] * distance[1]);
				}
			}
		}
